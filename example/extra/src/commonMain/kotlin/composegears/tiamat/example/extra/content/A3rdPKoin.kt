...
val A3rdPKoin by navDestination<Unit> {
    ...
    Screen("Koin ViewModel") {
        Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                VSpacer()
                Text("simpleViewModel -> ${simpleViewModel.getScopeId()} ")
                Text("paramViewModel -> ${paramViewModel.getScopeId()} ")
                Text("saveableViewModel -> ${saveableViewModel.getScopeId()} ")
                Text("sharedViewModel -> ${sharedViewModel.getScopeId()} ")
            }
        }
    }
}
